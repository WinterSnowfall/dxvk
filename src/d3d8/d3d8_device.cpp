...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D8Device::CreateImageSurface(
    ...
    // CreateImageSurface is guaranteed to succeed even with unsupported formats
    HRESULT res = m_parent->CheckDeviceFormat(0, D3DDEVTYPE_HAL, m_presentParams.BackBufferFormat,
                                              0, D3DRTYPE_SURFACE, Format);
    const bool placeInScratch = isD3D9ExclusiveFormat(Format) || FAILED(res);
    D3DPOOL pool = placeInScratch ? D3DPOOL_SCRATCH : D3DPOOL_SYSTEMMEM;
